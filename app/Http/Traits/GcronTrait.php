...
trait GcronTrait{
   ...
   $start=microtime(true);
     ...
     $this->id=$id;
   $this->data=DB::select('
   SELECT fusers.*,ifnull((invoice.vp*48),0) ammount,ifnull(vpcount.vp ,0) vp from fusers 
      left join (
      ...
      left join (
         select sum(vp) vp,fuser_id from invoices  group by fuser_id
      ) vpcount on vpcount.fuser_id=fusers.id
   ...
   ' ,['from_date'=>date('Y-m-d 00:00:00'),'to_date'=>date('Y-m-d 23:59:59')]);
    ...
    public function GenerationKeys($arr,$gen){
      ...
      //      $parcent=1;
      //   }
        $parcent=GenSetting::first()['gen'.$gen];
        $count=count($arr);
        $ammount=0;
        $my_key=array_keys($this->user,$this->id)[0];
        $user_vp=$this->data[$my_key]->vp;
      //   dd($user_vp);
