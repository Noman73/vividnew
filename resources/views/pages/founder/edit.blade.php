            ...
            <div class="col-md-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @endif
                        <form action="{{ route('founder.update') }}" method="POST" >@csrf
                            <input type="hidden" value="{{ $foudner_bal->id }}" name="founder_id">
                            ...
                            <select class="form-select form-control select2" aria-label="Default select example" style="width: 100%;" name="fuser_id" id="selectUser">
                                ...
                                <option value="{{  $foudner_bal->fuser_id }}" {{ $foudner_bal->fuser_id == $foudner_bal->fuser_id ? 'selected' : '' }}> {{ $foudner_bal->user->name }} ({{ $foudner_bal->user->username }})</option>
                            </select>
                              <div id="details" class="text-success"></div>
                              <div class="row mt-5">
                                  <div class="col-md-6">
                                      <label for="">Invest Amount</label>
                                      <input type="number" class="form-control" name="invest_amount" placeholder="Invest Amount" value="{{ isset($foudner_bal->invest_amount) ? $foudner_bal->invest_amount: 0 }}">
                                  </div>
                                  <div class="col-md-6">
                                      <label for="">Share Parcent</label>
                                      <input type="number" class="form-control" name="share" placeholder="Share Parcent" value="{{ isset($foudner_bal->share) ? $foudner_bal->share: 0 }}">
                                  </div>
                                    {{-- <div class="col-md-4">
